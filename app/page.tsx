...
export default function Home(){
 ...
 {tab==='Matching'&&<section className="panel full"><div className="panel-head"><div><small>GZ MATCHING ENGINE</small><h2>{selectedTalent?`Maisons compatibles · ${selectedTalent.nom}`:'Choisis un talent'}</h2></div></div>{selectedTalent?<div className="match-list">{selectedMatches.map(m=>{const h=getMaison(m.maison_id);return h&&<div className="match" key={`${m.talent_id}-${m.maison_id}`}><HouseLogo name={h.nom}/><div><b>{h.nom}</b><p>{m.recommandation||'Compatibilité stratégique GZ'}</p></div><strong>{Math.round(m.score)}%</strong></div>})}{selectedMatches.length===0&&<p className="sub">Pas encore de matching calculé pour ce profil.</p>}</div>:<div className="cards">{talents.slice(0,30).map(t=><button className="profile" key={t.id} onClick={()=>setSelectedTalent(t)}><WikiImage name={t.nom} src={t.image}/><b>{t.nom}</b><p>Voir les compatibilités →</p></button>)}</div>}</section>}
