...
export default function AssociateAccess() {
  ...
  return <main className="gz-access">
    <div className="gz-access-orb one"/><div className="gz-access-orb two"/>
    <section className="gz-access-card">
      ...
      <h1>Bienvenue dans<br/><em>l’espace Associé.</em></h1>
      <p className="gz-access-sub">Un accès privé à l’intelligence GZ — horlogerie, talents et matching.</p>
      <form onSubmit={submit}>
        <label>Prénom<input required value={first} onChange={e=>setFirst(e.target.value)} placeholder="Votre prénom"/></label>
        <label>Nom<input required value={last} onChange={e=>setLast(e.target.value)} placeholder="Votre nom"/></label>
        <label>Adresse e-mail<input required type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="prenom@domaine.com"/></label>
        <label>Créer votre mot de passe<input required type="password" minLength={8} value={password} onChange={e=>setPassword(e.target.value)} placeholder="8 caractères minimum"/></label>
        <button disabled={busy}>{busy ? 'Création de votre accès…' : 'Activer mon accès GZ  →'}</button>
      </form>
      {error && <p className="gz-access-error">{error}</p>}
      {message && <p className="gz-access-success">{message}</p>}
      <div className="gz-access-foot"><span>GZ AGENCY</span><span>CONFIDENTIAL · ASSOCIÉ</span></div>
    </section>
  </main>;
}
