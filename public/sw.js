const CACHE='gz-intelligence-v2';
const CORE=['/','/gz-icon.svg'];
self.addEventListener('install',event=>{event.waitUntil(caches.open(CACHE).then(cache=>cache.addAll(CORE)).then(()=>self.skipWaiting()))});
self.addEventListener('activate',event=>{event.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith('gz-intelligence-')&&k!==CACHE).map(k=>caches.delete(k)))).then(()=>self.clients.claim()))});
